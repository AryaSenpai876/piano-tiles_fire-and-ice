    ...
    return Container(
      decoration: const BoxDecoration(
        gradient: RadialGradient(
          colors: [Color(0xFFC099FF), Color(0xFF8B5CF6)],
      ...
      child: Stack(
        children: [
          Positioned(top: h*0.05, left: w*0.1, child: Container(width: w*0.2, height: w*0.2, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.034)))),
          Positioned(top: h*0.4, right: w*0.05, child: Container(width: w*0.3, height: w*0.3, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.025)))),
          Positioned(bottom: h*0.1, left: -w*0.05, child: Container(width: w*0.4, height: w*0.4, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.017)))),
          Positioned(bottom: -h*0.05, right: w*0.2, child: Container(width: w*0.15, height: w*0.15, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.042)))),
          Positioned(top: -h*0.1, right: w*0.3, child: Container(width: w*0.25, height: w*0.25, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.034)))),
          Positioned(top: h*0.2, left: w*0.4, child: Container(width: w*0.1, height: w*0.1, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.025)))),
          Positioned(top: h*0.6, left: w*0.15, child: Container(width: w*0.2, height: w*0.2, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.017)))),
          Positioned(bottom: h*0.25, right: w*0.1, child: Container(width: w*0.35, height: w*0.35, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.025)))),
          Positioned(top: h*0.7, right: w*0.4, child: Container(width: w*0.12, height: w*0.12, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.034)))),
          Positioned(top: -h*0.05, left: w*0.5, child: Container(width: w*0.18, height: w*0.18, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.042)))),
          Positioned(top: h*0.8, left: w*0.8, child: Container(width: w*0.2, height: w*0.2, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.03)))),
          Positioned(top: h*0.3, left: -w*0.1, child: Container(width: w*0.25, height: w*0.25, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withOpacity(0.02)))),
        ]
      )
    );
  }
...
class _CompanySplashScreenState extends State<CompanySplashScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SizedBox.expand(child: Image.asset('assets/images/splash_company.png', fit: BoxFit.cover,
        errorBuilder: (c, e, s) => Center(child: const Text("COMPANY", style: TextStyle(fontSize: 40, color: Colors.white, letterSpacing: 12))))
...
class _GameSplashScreenState extends State<GameSplashScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: Colors.black,
    body: SizedBox.expand(child: Image.asset('assets/images/splash_game.png', fit: BoxFit.cover,
        errorBuilder: (c, e, s) => Center(child: const Text("GAME LOGO", style: TextStyle(fontSize: 50, color: Colors.red, fontWeight: FontWeight.w900))))
...
class _MainMenuState extends State<MainMenu> with TickerProviderStateMixin {
  ...
  Future<void> _loadHighscore() async {
    if (kIsWeb) {
      final webScore = await WebHelper.loadHighscore();
      if (webScore != null) {
        setState(() => highscore = webScore);
      }
      return;
    }
    ...
    try {
      ...
      debugPrint('Error reading local highscore: $e');
    }
    ...
    if (!loadedFromLocal) {
      try {
        ...
        debugPrint('Error reading asset highscore: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFF9059FF),
      body: Stack(children: [
        ...
        Center(child: Column(children: [
          ...
          MouseRegion(
            onEnter: (_) => setState(() => isHoveringPlay = true),
            onExit: (_) => setState(() => isHoveringPlay = false),
            cursor: SystemMouseCursors.click,
            child: GestureDetector(onTap: () { previewPlayer.stop(); Navigator.push(context, MaterialPageRoute(builder: (c) => GameplayScreen(music: selectedMusic, map: selectedMap, bg: selectedBG, volume: volume))).then((_) => _loadHighscore()); },
                child: AnimatedContainer(
                  duration: 200.ms,
                  padding: EdgeInsets.symmetric(horizontal: w * 0.1, vertical: h * 0.025), 
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(colors: [Color(0xFFFF9500), Color(0xFFFF2A6D)], begin: Alignment.centerLeft, end: Alignment.centerRight), 
                    borderRadius: BorderRadius.circular(50), 
                    border: Border.all(color: const Color(0xFFFF6B6B), width: 3), 
                    boxShadow: [
                      BoxShadow(color: const Color(0xFFFF2A6D).withOpacity(0.5), blurRadius: isHoveringPlay ? 30 : 15, offset: const Offset(0, 8)),
                      BoxShadow(color: Colors.black.withOpacity(0.2), blurRadius: 10, offset: const Offset(0, 4)),
                    ]
                  ), 
                  transform: Matrix4.identity()..scale(isHoveringPlay ? 1.05 : 1.0),
                  transformAlignment: Alignment.center,
                  child: Text("PLAY", style: TextStyle(fontSize: h * 0.055, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 2))
                )),
          ),
  ...
  void _showMiniDialog(String title, Widget child) => showDialog(context: context, builder: (c) => Align(alignment: Alignment.bottomCenter, child: Padding(padding: const EdgeInsets.only(bottom: 120), child: Material(color: Colors.transparent, child: Container(width: 450, padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF8B5CF6), Color(0xFF6B21A8)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(25), border: Border.all(color: Colors.white.withOpacity(0.4), width: 2), boxShadow: [BoxShadow(color: const Color(0xFF8B5CF6).withOpacity(0.4), blurRadius: 20, spreadRadius: 2)]), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: 4)), Divider(color: Colors.white.withOpacity(0.2), height: 25), child]))))).animate().scale(curve: Curves.easeOutBack, duration: 400.ms));

  Widget _buildSelectorItem(String name, bool isSelected, VoidCallback onTap) => Container(margin: const EdgeInsets.symmetric(vertical: 5), decoration: BoxDecoration(color: isSelected ? Colors.white.withOpacity(0.2) : Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(15), border: Border.all(color: isSelected ? Colors.white : Colors.white10, width: 2), boxShadow: isSelected ? [BoxShadow(color: Colors.white.withOpacity(0.2), blurRadius: 8)] : []), child: ListTile(title: Text(name, style: TextStyle(color: isSelected ? Colors.white : Colors.white60, fontSize: 16, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)), trailing: isSelected ? const Icon(Icons.check_circle, color: Colors.white, size: 28) : null, onTap: onTap));
  ...
  void _showSettingsDialog() => _showMiniDialog("SETTINGS", StatefulBuilder(builder: (context, setD) => Column(children: [Row(children: [Icon(volume > 0.5 ? Icons.volume_up : Icons.volume_down, color: Colors.white), Expanded(child: Slider(value: volume, activeColor: Colors.white, inactiveColor: Colors.white.withOpacity(0.3), onChanged: (v) { setD(() => volume = v); setState(() => volume = v); previewPlayer.setVolume(v); }))]), Text("SYSTEM VOLUME", style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 14, letterSpacing: 3, fontWeight: FontWeight.bold))])));
}
...
class _GameplayScreenState extends State<GameplayScreen> {
  ...
  void _handleTap(int col) {
    ...
    if (i != -1) {
      setState(() { score += 1; tiles.removeAt(i); if (col == 0) leftFlashOpacity = 0.2; else rightFlashOpacity = 0.2; });
  ...
  Future<void> _saveHighscore() async {
    if (kIsWeb) {
      await WebHelper.saveHighscore(score);
      return;
    }
    try {
      final dir = await getApplicationDocumentsDirectory();
      ...
      if (await file.exists()) {
        try {
          ...
          debugPrint('Error reading local highscore during save: $e');
        }
      } 
      ...
      if (!readFromLocal) {
        try {
          ...
          debugPrint('Error reading asset highscore during save: $e');
        }
      }
      
      // Always save the last played score, as requested
      await file.writeAsString(jsonEncode({'highscore': score}));
    } catch (e) {
      debugPrint('Error in _saveHighscore: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading) {
      return Scaffold(body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Text("GET READY", style: TextStyle(fontSize: h * 0.08, fontWeight: FontWeight.bold, letterSpacing: 10)),
        SizedBox(height: h * 0.05),
        Container(width: w * 0.4, height: h * 0.02, decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(5)), child: Align(alignment: Alignment.centerLeft, child: Container(width: 0, height: h * 0.02, color: Colors.red).animate().custom(duration: 2.seconds, builder: (c, v, ch) => Container(width: (w * 0.4) * v, height: h * 0.02, color: Colors.red)))),
      ])));
    }
    return Scaffold(
      backgroundColor: const Color(0xFF9059FF),
    body: KeyboardListener(focusNode: _focusNode..requestFocus(), onKeyEvent: (event) { if (event is KeyDownEvent) { if (event.logicalKey == LogicalKeyboardKey.keyQ) _handleTap(0); if (event.logicalKey == LogicalKeyboardKey.keyE) _handleTap(1); if (event.logicalKey == LogicalKeyboardKey.space) _startGame(); } },
      child: Stack(children: [
        if (widget.bg == null) const SizedBox.expand(child: BokehBackground()),
        if (widget.bg != null) ...[
